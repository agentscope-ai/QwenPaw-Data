...
Initialize the recommended local full-stack environment:
  ...
  2. Hash-locked QwenPaw Data workspace dependencies and editable packages
  3. DataBridge MCP client for the QwenPaw Data Host workspace
...
Options:
  ...
  --skip-cli-link             Do not expose qwenpaw-data in the user CLI bin directory
  -h, --help                  Show this help

Environment:
  QWENPAW_DATA_ENV_FILE            Optional dotenv file. Default: repository root .env
  QWENPAW_DATA_HOME                QwenPaw Data home directory. Default: ~/.qwenpaw-data
  QWENPAW_DATA_CM_BASE_URL         DataBridge API origin. Default: http://127.0.0.1:8765
  QWENPAW_DATA_CLI_BIN_DIR         Directory for the qwenpaw-data command link. Default: ~/.local/bin
...
workspace_requirements="$(mktemp "${TMPDIR:-/tmp}/qwenpaw-data-workspace-requirements.XXXXXX")"
mcp_import_file=""
cleanup_init_files() {
...
run "${uv_cmd}" pip install \
  --python "${workspace_python}" \
  --no-deps \
  --editable "${repo_root}/packages/qwenpaw-data-context" \
  --editable "${repo_root}/packages/qwenpaw-data-host-core" \
  --editable "${repo_root}/packages/qwenpaw-data-cli" \
  --editable "${repo_root}/packages/qwenpaw-data-skills"

workspace_qwenpaw_data="${repo_root}/.venv/bin/qwenpaw-data"
if [[ ! -x "${workspace_qwenpaw_data}" ]]; then
  echo "Expected QwenPaw Data CLI was not installed: ${workspace_qwenpaw_data}" >&2
  exit 1
fi
...
cli_link=""
install_cli_link() {
  cli_bin_dir="$(qwenpaw_data_abs_path "${QWENPAW_DATA_CLI_BIN_DIR:-~/.local/bin}")"
  cli_link="${cli_bin_dir}/qwenpaw-data"

  run mkdir -p "${cli_bin_dir}"
  if [[ -L "${cli_link}" ]]; then
    local existing_target
    existing_target="$(readlink "${cli_link}")"
    if [[ "${existing_target}" == "${workspace_qwenpaw_data}" ]]; then
      printf 'QwenPaw Data CLI command already linked: %s\n' "${cli_link}"
      return
    fi
    if [[ "${existing_target}" == "${repo_root}"/* ]]; then
      # 指向本仓库内旧安装位置（如已废弃的子包 .venv），视为残留并更新。
      echo "Updating stale QwenPaw Data CLI link: ${cli_link} -> ${workspace_qwenpaw_data}"
      run ln -sf "${workspace_qwenpaw_data}" "${cli_link}"
      return
    fi
    echo "Refusing to replace existing QwenPaw Data CLI link: ${cli_link} -> ${existing_target}" >&2
    echo "Remove it explicitly or rerun with --skip-cli-link." >&2
    exit 1
  fi
  if [[ -e "${cli_link}" ]]; then
    echo "Refusing to replace existing QwenPaw Data CLI command: ${cli_link}" >&2
    echo "Remove it explicitly or rerun with --skip-cli-link." >&2
    exit 1
  fi

  run ln -s "${workspace_qwenpaw_data}" "${cli_link}"
}
...
if [[ "${skip_mcp_config}" != "1" ]]; then
  cm_base_url="${QWENPAW_DATA_CM_BASE_URL:-http://127.0.0.1:8765}"
  while [[ "${cm_base_url}" == */ ]]; do
    cm_base_url="${cm_base_url%/}"
  done
  if [[ -z "${cm_base_url}" ]]; then
    echo "QWENPAW_DATA_CM_BASE_URL must not be empty." >&2
    exit 1
  fi

  mcp_import_file="$(mktemp "${TMPDIR:-/tmp}/qwenpaw-data-databridge-mcp.XXXXXX")"
  (
    umask 077
...
EOF
  ...
  run "${workspace_qwenpaw_data}" mcp import "${mcp_import_file}"
fi

printf 'Local initialization complete.\n'
if [[ -n "${cli_link}" ]]; then
  printf 'QwenPaw Data CLI command: %s\n' "${cli_link}"
  case ":${PATH}:" in
    *":${cli_bin_dir}:"*) ;;
    *)
