"""Doctor command: environment self-check for DataPaw."""
...
def _check_model_config() -> CheckResult:
    model = (os.getenv("DATAPAW_MODEL_NAME") or os.getenv("LLM_MODEL") or "").strip()
    api_key = (
        os.getenv("DATAPAW_MODEL_API_KEY") or os.getenv("OPENAI_API_KEY") or ""
    ).strip()
    provider = (os.getenv("DATAPAW_MODEL_PROVIDER") or "openai").strip().lower()
    missing = [
    ...
    if missing:
        return CheckResult(
            ...
            ["Set LLM_MODEL and OPENAI_API_KEY (or DATAPAW_MODEL_* overrides)."],
        )
    return CheckResult(
...
def _check_auth_config() -> CheckResult:
    server_token = bool((os.getenv("DATAPAW_API_TOKEN") or "").strip())
    scoped_keys = bool((os.getenv("DATAPAW_API_KEYS") or "").strip())
    client_token = bool((os.getenv("DATAPAW_CLIENT_API_TOKEN") or "").strip())
    ...
    if not server_auth:
        return CheckResult(
            ...
            ["Set DATAPAW_API_KEYS before exposing any service beyond 127.0.0.1."],
        )
    if not client_token:
        return CheckResult(
            "API authentication",
            ...
            ["Set DATAPAW_CLIENT_API_TOKEN to a matching scoped key."],
        )
...
def _check_datapaw_home() -> CheckResult:
    from datapaw.host.core import resolve_datapaw_home
    ...
    return CheckResult(
        ...
        [] if writable else [f"Grant write access to {probe} or set DATAPAW_HOME."],
    )
...
def _check_mcp_config() -> CheckResult:
    ...
    path = resolve_datapaw_home() / "host" / "workspace" / ".mcp"
    if not path.is_file():
        return CheckResult(
            "DataBridge MCP",
            _WARN,
...
def _check_docker_daemon() -> CheckResult:
    ...
    if workspace_type == "local":
        ...
        hints.append(
            "如需临时绕过 Docker，只能显式设置 DATAPAW_WORKSPACE=local（不安全）。"
        )
    return CheckResult(
        "Docker daemon",
...
def _check_databridge() -> CheckResult:
    base_url = (os.getenv("DATAPAW_CM_BASE_URL") or "http://127.0.0.1:8765").rstrip("/")
    import httpx
...
def _check_workspace_backend() -> CheckResult:
    ...
    return CheckResult(
        "Workspace backend",
        _INFO,
        f"{workspace_type} (override via --workspace / DATAPAW_WORKSPACE)",
    )
