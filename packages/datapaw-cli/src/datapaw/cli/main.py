...
from datapaw.cli.env import load_datapaw_env
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"datapaw: error: unable to initialize log file: {exc}", file=sys.stderr)
        return 1
    ...
    try:
        ...
        logger.exception(
            "DataPaw CLI command failed: command=%s",
            getattr(args, "command", None) or "-",
        )
        for line in _flatten_exception(exc):
            print(f"datapaw: error: {line}", file=sys.stderr)
        return 1
...
def build_parser(*, include_internal: bool = False) -> argparse.ArgumentParser:
    from datapaw.cli.commands import COMMANDS

    parser = argparse.ArgumentParser(prog="datapaw")
    subparsers = parser.add_subparsers(dest="command")
    for command in COMMANDS:
        command.register(subparsers)
    if include_internal:
        from datapaw.cli.commands import mcp
