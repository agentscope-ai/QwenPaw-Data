...
def ingest_trace_events(req: TraceIngestRequest, request: Request):
    ...
    from ..graph.trace import ingest_datapaw_trace
...
def ingest_trace_file(
    ...
    try:
        ...
        max_line_bytes = int(os.getenv("DATAPAW_TRACE_MAX_LINE_BYTES", str(1024 * 1024)))
        max_events = int(os.getenv("DATAPAW_TRACE_MAX_EVENTS", "100000"))
        ...
        with upload_path.open("rb") as handle:
            while True:
                ...
                if len(raw_line) > max_line_bytes and not raw_line.endswith(b"\n"):
                    ...
                    raise HTTPException(413, "JSONL 单行超过 DATAPAW_TRACE_MAX_LINE_BYTES")
                line = raw_line.strip()
                if not line:
                    continue
                try:
                ...
                if event_count > max_events:
                    ...
                    raise HTTPException(413, "Trace 事件数超过 DATAPAW_TRACE_MAX_EVENTS")
...
# ------------------------------------------------------------------ #
# submit_trace — DataPaw Host snapshot
# ------------------------------------------------------------------ #

@callback_router.post("/submit_trace")
def submit_trace(payload: dict[str, Any], request: Request):
    """Parse and persist one complete DataPaw Host session snapshot."""
    from fastapi.responses import JSONResponse
