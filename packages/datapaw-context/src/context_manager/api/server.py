...
async def _event_loop_lag_monitor(app: FastAPI) -> None:
    ...
    interval = _positive_float_env("DATAPAW_EVENT_LOOP_PROBE_SECONDS", 1.0, 0.01)
    warning = _positive_float_env("DATAPAW_EVENT_LOOP_LAG_WARN_SECONDS", 0.2, 0.001)
...
from datapaw.context.paths import access_log_path, data_bridge_logs_dir
...
def create_app() -> FastAPI:
    ...
    access_log_handler = AsyncRotatingFileHandler(
        ...
        backup_count=2,
        queue_size=_positive_int_env("DATAPAW_ACCESS_LOG_QUEUE_SIZE", 4096),
    )
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            ...
            lag_monitor = asyncio.create_task(
                ...
                name="datapaw-event-loop-lag-monitor",
            )
    ...
    app = FastAPI(title="DataPaw Context Manager", version="0.1.0", lifespan=lifespan)
    ...
    from datapaw.context.resource_budget import ResourceBudgetMiddleware
    from datapaw.context.uploads import RequestBodyLimitMiddleware
