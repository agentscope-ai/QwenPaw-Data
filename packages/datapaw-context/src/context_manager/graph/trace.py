...
def _parse_datapaw_events(events: list[dict], *, goal: str = "") -> dict:
    """解析 JSONL 事件流。
...
def ingest_datapaw_trace(
    ...
    *,
    ...
    parsed = _parse_datapaw_events(events, goal=goal)
    steps = parsed["steps"]
    goal = parsed["goal"]
    final_answer = parsed["final_answer"]

    if not goal:
        log.warning("ingest_datapaw_trace: no goal found, skipping")
    ...
    log.info(
        "ingest_datapaw_trace: task=%s goal=%.60s steps=%d status=%s",
        task_key, goal, len(steps), status,
    )
    ...
    with neo4j_session(driver) as s:
        ...
        task_dict = {
            ...
            "goal": goal[:1000],
            "status": str(status or "success"),
            "owner_agent": str(owner_agent or "datapaw"),
            "task_signature": signature_hash,
            "as_of_date": as_of_date or "",
            "created_at": _iso(_dt.datetime.now(_dt.timezone.utc)),
            "failure_lesson": str(failure_lesson or "")[:1000],
    ...
    if auto_distill and status in ("success", "failed"):
        try:
            ...
            log.warning("ingest_datapaw_trace: claim distill failed for %s: %s", task_key, exc)
...
# ---------------------------------------------------------------------- #
# DataPaw Host snapshot ingestion
# ---------------------------------------------------------------------- #

def _write_host_trace_tx(tx, *, model: dict[str, Any]) -> None:
    """Merge a parsed DataPaw host trace into the TG."""
...
def ingest_host_trace(
    ...
    *,
    ...
    """Parse and atomically persist a complete DataPaw host session snapshot.
