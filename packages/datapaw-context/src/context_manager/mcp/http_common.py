...
def _mcp_auth_settings() -> tuple[TokenVerifier | None, AuthSettings | None]:
    ...
    resource_url = (
        os.environ.get("DATAPAW_MCP_PUBLIC_URL")
        or "http://127.0.0.1:8765/mcp/v1/cm"
    ).strip()
    parsed = urlsplit(resource_url)
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise ValueError("DATAPAW_MCP_PUBLIC_URL must be an absolute HTTP(S) URL")
...
def _transport_security() -> TransportSecuritySettings:
    ...
    allowed_hosts = [
        ...
        "host.docker.internal:*",
        *_csv_env("DATAPAW_MCP_ALLOWED_HOSTS"),
    ]
    extra_origins_raw = os.environ.get("DATAPAW_MCP_ALLOWED_ORIGINS") or ""
    extra_origins = (
...
def create_streamable_mcp(name: str) -> FastMCP:
    ...
    The bind host defaults to loopback; set ``DATAPAW_MCP_HOST=0.0.0.0`` to
    ...
    return FastMCP(
        ...
        host=(os.environ.get("DATAPAW_MCP_HOST") or "127.0.0.1").strip(),
...
class _McpMount:
    ...
    async def _call_http_logged(
        ...
        for key, value in scope.get("headers") or []:
            if key.lower() in (b"x-datapaw-run", b"x-request-id"):
