"""上传大小统一限制（datapaw.context.uploads）的回归测试。"""
...
from datapaw.context.uploads import (
...
def test_default_limit_is_50mb(monkeypatch):
    monkeypatch.delenv("DATAPAW_MAX_UPLOAD_MB", raising=False)
    assert max_upload_bytes() == 50 * 1024 * 1024


def test_env_override_and_invalid_value(monkeypatch):
    monkeypatch.setenv("DATAPAW_MAX_UPLOAD_MB", "2")
    assert max_upload_bytes() == 2 * 1024 * 1024
    monkeypatch.setenv("DATAPAW_MAX_UPLOAD_MB", "not-a-number")
    assert max_upload_bytes() == 50 * 1024 * 1024


def test_upload_within_limit_ok(monkeypatch):
    monkeypatch.setenv("DATAPAW_MAX_UPLOAD_MB", "1")
...
def test_upload_over_limit_returns_413(monkeypatch):
    ...
    assert "DATAPAW_MAX_UPLOAD_MB" in resp.json()["detail"]
