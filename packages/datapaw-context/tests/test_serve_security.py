...
_SPEC = importlib.util.spec_from_file_location("datapaw_context_serve", _SERVE_PATH)
...
def test_external_host_detection() -> None:
    ...
    assert not _SERVE._is_loopback_host("datapaw.example.com")
...
def test_scoped_keys_count_as_external_bind_authentication(monkeypatch) -> None:
    ...
    monkeypatch.setenv("DATAPAW_API_KEYS", '[{"id":"reader"}]')
    assert _SERVE._authentication_configured()


def test_http_concurrency_limit_is_positive_and_configurable(monkeypatch) -> None:
    monkeypatch.setenv("DATAPAW_HTTP_MAX_CONCURRENCY", "64")
    assert _SERVE._positive_int_env("DATAPAW_HTTP_MAX_CONCURRENCY", 128) == 64
    monkeypatch.setenv("DATAPAW_HTTP_MAX_CONCURRENCY", "0")
    assert _SERVE._positive_int_env("DATAPAW_HTTP_MAX_CONCURRENCY", 128) == 1
    monkeypatch.setenv("DATAPAW_HTTP_MAX_CONCURRENCY", "invalid")
    assert _SERVE._positive_int_env("DATAPAW_HTTP_MAX_CONCURRENCY", 128) == 128
