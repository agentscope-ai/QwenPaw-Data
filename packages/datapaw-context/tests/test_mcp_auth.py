...
async def test_mcp_uses_native_token_verifier(
    ...
    monkeypatch.setenv("DATAPAW_API_TOKEN", "mcp-secret")
...
def test_mcp_transport_uses_exact_origin_allowlist(
    ...
    monkeypatch.setenv("DATAPAW_CORS_ORIGINS", "http://localhost:3000")
    monkeypatch.delenv("DATAPAW_MCP_ALLOWED_ORIGINS", raising=False)
    settings = _transport_security()
    assert settings.allowed_origins == ["http://localhost:3000"]

    monkeypatch.setenv("DATAPAW_MCP_ALLOWED_ORIGINS", "*")
...
def test_standalone_mcp_reuses_api_edge_security(
    ...
    monkeypatch.setenv("DATAPAW_API_TOKEN", "standalone-mcp-token")
    monkeypatch.delenv(API_KEYS_ENV, raising=False)
    monkeypatch.delenv("DATAPAW_API_ALLOWED_HOSTS", raising=False)
    monkeypatch.setenv(
        "DATAPAW_SECURITY_AUDIT_LOG",
