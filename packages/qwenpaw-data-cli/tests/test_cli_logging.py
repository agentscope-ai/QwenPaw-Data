...
from qwenpaw_data.cli import logging_config
...
def _owned_handlers() -> list[logging.Handler]:
    return [
        ...
        if isinstance(handler, logging_config._QwenPawDataCLIFileHandler)
    ]
...
def test_configure_cli_logging_routes_root_and_agentscope_to_file(
    ...
    home = tmp_path / "home"
    monkeypatch.setenv("QWENPAW_DATA_HOME", str(home))
    ...
    logging.getLogger(
        "qwenpaw_data.host.core.orchestration.middleware",
    ).info("qwenpaw-data-trace-marker")
    _flush_owned_handlers()

    captured = capsys.readouterr()
    content = log_path.read_text(encoding="utf-8")

    assert log_path == (home / "host" / "qwenpaw_data.log").resolve()
    assert captured.err == ""
    assert "root-info-marker" in content
    assert "agentscope-info-marker" in content
    assert "qwenpaw-data-trace-marker" in content
...
def test_configure_cli_logging_is_idempotent_and_tracks_home_changes(
    ...
    monkeypatch.setenv("QWENPAW_DATA_HOME", str(first_home))
    ...
    logging.getLogger("qwenpaw_data.test").info("written-once-marker")
    _flush_owned_handlers()
    assert (
    ...
    monkeypatch.setenv("QWENPAW_DATA_HOME", str(second_home))
    second_path = configure_cli_logging()

    [second_handler] = _owned_handlers()
    assert second_path == (second_home / "host" / "qwenpaw_data.log").resolve()
...
def test_cli_log_rotates(
    ...
    monkeypatch.setenv("QWENPAW_DATA_HOME", str(tmp_path / "home"))
    monkeypatch.setattr(logging_config, "_MAX_BYTES", 256)

    log_path = configure_cli_logging()
    for index in range(12):
        logging.getLogger("qwenpaw_data.rotation").info(
    ...
    assert log_path.with_name("qwenpaw_data.log.1").is_file()
    assert log_path.with_name("qwenpaw_data.log.2").is_file()
...
"""
    ...
    assert result == {
        "log_path": str((home / "host" / "qwenpaw_data.log").resolve()),
        "terminal_handlers": [],
        "agentscope_handlers": 0,
    }
    content = (home / "host" / "qwenpaw_data.log").read_text(encoding="utf-8")
    assert "runtime-root-marker" in content
    assert "runtime-agentscope-marker" in content
    assert "runtime-qwenpaw-data-trace-marker" in content
...
def test_main_prints_wrapped_error_and_logs_traceback(
    ...
    captured = capsys.readouterr()
    assert captured.out == ""
    assert captured.err == "qwenpaw-data: error: RuntimeError: command exploded\n"

    log_path = Path(os.environ["QWENPAW_DATA_HOME"]) / "host" / "qwenpaw_data.log"
    _flush_owned_handlers()
    content = log_path.read_text(encoding="utf-8")
    assert "QwenPaw Data CLI command failed: command=run" in content
...
def test_main_reports_log_initialization_failure(
    ...
    assert captured.out == ""
    assert captured.err == (
        "qwenpaw-data: error: unable to initialize log file: permission denied\n"
    )
...
def test_stream_renderer_remains_explicit_stdout(
    ...
    from qwenpaw_data.cli.util import print_event_stream
