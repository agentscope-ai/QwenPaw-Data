...
from qwenpaw_data.host.core.paths import host_root

_LOG_FILENAME = "qwenpaw_data.log"
...
class _QwenPawDataCLIFileHandler(RotatingFileHandler):
    """Marker type for the file handler owned by the QwenPaw Data CLI."""
...
def _configure_root_logger(log_path: Path) -> _QwenPawDataCLIFileHandler:
    root = logging.getLogger()
    owned_handler: _QwenPawDataCLIFileHandler | None = None

    for handler in list(root.handlers):
        if isinstance(handler, _QwenPawDataCLIFileHandler):
    ...
    if owned_handler is None:
        owned_handler = _QwenPawDataCLIFileHandler(
            log_path,
            maxBytes=_MAX_BYTES,
...
def _route_agentscope_logger_to_root() -> None:
    ...
    # through the root so workspace initialization logs use qwenpaw_data.log too.
...
def configure_cli_logging() -> Path:
    """Route CLI process logs to ``QWENPAW_DATA_HOME/host/qwenpaw_data.log``."""
