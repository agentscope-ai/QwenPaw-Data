"""Shared helpers for QwenPaw Data CLI commands."""
...
def add_permission_mode_arg(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        help=(
            ...
            "QWENPAW_DATA_PERMISSION_MODE."
        ),
    )


def resolve_workspace_type(args: argparse.Namespace | None = None) -> str:
    """CLI 参数 > QWENPAW_DATA_WORKSPACE 环境变量 > docker；非法值直接报错。"""
    from_args = getattr(args, "workspace", None) if args is not None else None
    value = (from_args or os.getenv("QWENPAW_DATA_WORKSPACE", "") or "docker").strip().lower()
...
def resolve_permission_mode(
    ...
    *,
    ...
    value = (
        from_args
        or os.getenv("QWENPAW_DATA_PERMISSION_MODE", "")
        or "auto"
    ).strip().lower()
    ...
    async def confirm(event: Any) -> Any:
        ...
        for tool_call in event.tool_calls:
            ...
            if interactive:
                ...
                prompt = (
                    "\nQwenPaw Data permission request\n"
...
def create_qwenpaw_data(
    request_context: dict[str, Any] | None = None,
    workspace_type: str | None = None,
    permission_mode: str | None = None,
    confirmation_handler: Any = None,
) -> Any:
    from qwenpaw_data.host.core import QwenPawDataHost, resolve_qwenpaw_data_home
