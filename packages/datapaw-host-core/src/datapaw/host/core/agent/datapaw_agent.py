"""Standalone DataPaw agent (AgentScope 2.0)."""
...
from ..orchestration.middleware import (
    DataPawHintMiddleware,
    DataPawPromptMiddleware,
    DataPawReplyMiddleware,
    DataPawTraceMiddleware,
)
...
class DataPawAgent(Agent):
    """Standalone DataPaw agent built on AgentScope 2.0 ``Agent``."""
    ...
    def __init__(
        ...
        *,
        ...
        self._cm_mcp_tool_prefixes = set(
            getattr(toolkit, "_datapaw_cm_mcp_tool_prefixes", set()),
        )
    ...
    def set_mode(self, new_mode: str) -> None:
        if new_mode not in ("agent", "plan"):
            logger.warning("DataPawAgent.set_mode: invalid mode %r", new_mode)
            return
        self._mode = new_mode  # type: ignore[assignment]
        self.state.tool_context.activated_groups = [new_mode]
        set_datapaw_mode = getattr(self.toolkit, "set_datapaw_mode", None)
        if callable(set_datapaw_mode):
            set_datapaw_mode(new_mode)
    ...
    async def _collect_reply(
        ...
        if final_msg is None:
            raise RuntimeError("DataPawAgent did not produce a final message.")
    ...
    def _build_middlewares(self) -> list[MiddlewareBase]:
        """Build the DataPaw middleware stack."""
        rs = self._runtime_state
        return [
            DataPawReplyMiddleware(rs),
            DataPawTraceMiddleware(
                rs,
