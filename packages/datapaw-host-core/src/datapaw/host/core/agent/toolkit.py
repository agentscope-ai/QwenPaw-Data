...
from ..orchestration.tools import PLAN_MODE_TOOL_NAMES, build_datapaw_tools
...
# ``DATAPAW_MCP_EXECUTION_TIMEOUT`` (<= 0 disables the fallback).
DEFAULT_MCP_EXECUTION_TIMEOUT = 120.0


def _default_mcp_execution_timeout() -> float:
    raw = os.environ.get("DATAPAW_MCP_EXECUTION_TIMEOUT", "")
    try:
...
def _mcp_discovery_timeout() -> float:
    raw = os.environ.get("DATAPAW_MCP_DISCOVERY_TIMEOUT", "")
    try:
...
def _inject_run_header(mcps: list[Any]) -> None:
    ...
    DataBridge's protocol log records the ``x-datapaw-run`` header, so lines
    ...
    """
    api_token = (
        (os.environ.get("DATAPAW_CLIENT_API_TOKEN") or "").strip()
        or (os.environ.get("DATAPAW_API_TOKEN") or "").strip()
    )
    for client in mcps:
...
class DataPawToolkit(Toolkit):
    """Toolkit with DataPaw's externally controlled mode boundary."""

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        super().__init__(*args, **kwargs)
        self._datapaw_mode: str | None = None

    def set_datapaw_mode(self, mode: str) -> None:
        self._datapaw_mode = mode if mode in {"plan", "agent"} else None

    async def _get_available_tools(self, groups: list[str] | None) -> dict:
        effective_groups = (
            [self._datapaw_mode]
            if self._datapaw_mode in {"plan", "agent"}
            else groups
        )
...
async def build_datapaw_toolkit(
    ...
    *,
    ...
    """拼装 DataPaw Agent 用的 grouped ``Toolkit``。"""
    shared_tools = list(build_datapaw_tools(runtime_state, mode="plan"))
    agent_only_tools = [
        tool
        for tool in build_datapaw_tools(runtime_state, mode="agent")
    ...
    # Tag HTTP clients with this process's run id (x-datapaw-run) so the
    ...
    toolkit = DataPawToolkit(
        ...
        tool_groups=[
            ToolGroup(
                name="plan",
                description="Tools for creating and revising a DataPaw plan.",
                instructions=(
            ...
            ToolGroup(
                name="agent",
                description="Tools for executing a DataPaw DAG and producing artifacts.",
                instructions=(
    ...
    toolkit._datapaw_cm_mcp_tool_prefixes = cm_mcp_tool_prefixes
    return toolkit
