# -*- coding: utf-8 -*-
"""DataPaw ToolBase 子类 —— agentscope 2.0 工具协议实现。
...
class DataPawToolBase(ToolBase):
    """DataPaw 工具基类。构造时接收 RuntimeStateManager 引用。"""
...
class CreatePlan(DataPawToolBase):
...
class UpdateSubtask(DataPawToolBase):
...
class ReviseCurrentPlan(DataPawToolBase):
...
class FinishPlan(DataPawToolBase):
...
def build_datapaw_tools(
    runtime_state: RuntimeStateManager,
    mode: str = "agent",
) -> list[ToolBase]:
    """Build DataPaw tool instances for the given mode."""
    tools = [cls(runtime_state) for cls in ALL_DATAPAW_TOOLS]
    if mode == "plan":
