# -*- coding: utf-8 -*-
"""DataPaw Middleware 集合 —— agentscope 2.0 MiddlewareBase 实现。

5 个 Middleware 替代 v1.x 的 mixin override：
- DataPawPromptMiddleware: 构建 master prompt + env hint (on_system_prompt)
- DataPawHintMiddleware: 追加 DAG 状态提示 (on_system_prompt)
- DataPawReplyMiddleware: pending edits 注入 + trigger_msg_id (on_reply)
- DataPawTraceMiddleware: trace 收集 (on_reasoning + on_acting)
- DataPawSandboxContextMiddleware: sandbox context 同步 (on_acting)
"""
...
class DataPawPromptMiddleware(MiddlewareBase):
    """构建 DataPaw master prompt + mode prompt + environment hint。
...
class DataPawHintMiddleware(MiddlewareBase):
...
class DataPawReplyMiddleware(MiddlewareBase):
    ...
    async def _inject_pending_edits(self, agent: "Agent") -> None:
        ...
        try:
            ...
            logger.warning("Failed to inject DataPaw pending edits", exc_info=True)


class DataPawTraceMiddleware(MiddlewareBase):
    ...
    def _record_block_trace(
        ...
        *,
        ...
        logger.info(
            "DataPaw trace phase=%s graph_id=%s node_id=%s %s=%s",
            phase,
    ...
    def _record_tool_call_trace(
        ...
        *,
        ...
        logger.info(
            "DataPaw trace phase=%s graph_id=%s node_id=%s "
    ...
    async def _append_session_trace(
        ...
        *,
        ...
        try:
            ...
            logger.warning("Failed to append DataPaw session trace", exc_info=True)


class DataPawSandboxContextMiddleware(MiddlewareBase):
    ...
    def _sync_context(self, tool_call: Any) -> None:
        ...
        try:
            ...
            logger.warning(
                "DataPaw sandbox rejected rel_path %r", rel_path, exc_info=True,
            )
