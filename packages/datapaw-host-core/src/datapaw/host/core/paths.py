...
DATAPAW_HOME_ENV = "DATAPAW_HOME"
_DEFAULT_HOME = Path("~/.datapaw")


def resolve_datapaw_home(home: str | Path | None = None) -> Path:
    """解析 DataPaw 根目录。

    优先级：显式参数 > ``DATAPAW_HOME`` 环境变量 > 默认 ``~/.datapaw``。
    """
    if home is not None:
        candidate = Path(home)
    else:
        raw = (os.environ.get(DATAPAW_HOME_ENV) or "").strip()
...
def host_root(home: str | Path | None = None) -> Path:
    """解析 Host 顶层状态目录。"""
    return resolve_datapaw_home(home) / "host"
...
class Paths:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "home", resolve_datapaw_home(self.home))
