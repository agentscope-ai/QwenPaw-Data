...
class ManagedDockerBash(Bash):
    ...
    async def call(
        ...
        pidfile = f"/tmp/datapaw-exec-{uuid.uuid4().hex}.pid"
    ...
    async def _run_cleanup(self, pidfile: str) -> tuple[bool, str]:
        ...
        chunks = [
            ...
            async for chunk in self._delegate.call(
                command=command,
                description="Reap interrupted DataPaw command",
...
def _source_skill_paths() -> list[str]:
    ...
    return _discover_skill_paths(package_root / "datapaw-skills" / "skills")


def _installed_skill_paths() -> list[str]:
    try:
        dist = distribution("datapaw-skills")
    except PackageNotFoundError:
        return []
    return _discover_skill_paths(Path(dist.locate_file("datapaw_skills/skills")))
...
class DataPawLocalWorkspace(LocalWorkspace):
    """Local workspace with DataPaw workspace-scoped filesystem tools."""
    ...
    async def list_tools(self) -> list[ToolBase]:
        ...
        # LocalBackend on Windows. Keep DataPaw's POSIX process-group adapter on
...
def create_docker_workspace(
    ...
    镜像与依赖可通过环境变量调整：
    - ``DATAPAW_DOCKER_BASE_IMAGE``：基础镜像（默认 python:3.11-slim）
    - ``DATAPAW_DOCKER_EXTRA_PIP``：逗号分隔，追加在内置分析栈之后
    - ``DATAPAW_DOCKER_HOST_ALIAS``：容器内访问宿主机服务的主机名
    ...
    try:
        ...
        pass
    for name in os.getenv("DATAPAW_DOCKER_EXTRA_PIP", "").split(","):
        name = name.strip()
    ...
    workdir = paths.workspace
    workdir.mkdir(parents=True, exist_ok=True)
    return DataPawDockerWorkspace(
        base_image=os.getenv("DATAPAW_DOCKER_BASE_IMAGE", "").strip()
        or "python:3.11-slim",
...
def _container_mcp_specs(
    ...
    api_token = (os.environ.get("DATAPAW_CLIENT_API_TOKEN") or "").strip() or (
        os.environ.get("DATAPAW_API_TOKEN") or ""
    ).strip()
    if not api_token:
        return prepared
...
def _make_datapaw_docker_workspace_cls() -> type:
    """延迟构造 DockerWorkspace 子类，避免 import 时引入 aiodocker 依赖。"""
    from agentscope.workspace import DockerWorkspace

    class _DataPawDockerWorkspace(DockerWorkspace):
        ...
        def _alias() -> str:
            return (
                os.getenv("DATAPAW_DOCKER_HOST_ALIAS", "").strip()
    ...
    return _DataPawDockerWorkspace


def DataPawDockerWorkspace(**kwargs: Any) -> Any:  # noqa: N802
    """工厂函数形态的入口，保持调用侧类似构造器的用法。"""
    cls = _make_datapaw_docker_workspace_cls()
    return cls(**kwargs)
