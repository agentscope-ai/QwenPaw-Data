...
def workspace_modules() -> object:
    ...
    from datapaw.host.core.utils import workspace as workspace_module
...
def test_default_skill_paths_discovers_hierarchical_skills(
    ...
    skill_root = Path(__file__).resolve().parents[2] / "datapaw-skills" / "skills"
...
def test_default_skill_paths_falls_back_to_installed_distribution(
    ...
    _, workspace_module = workspace_modules
    flat_skill = tmp_path / "datapaw_skills" / "skills" / "flat-skill"
    installed_skill = (
        tmp_path / "datapaw_skills" / "skills" / "workflows" / "installed-skill"
    )
...
@pytest.mark.asyncio
async def test_local_workspace_seeds_datapaw_skills(
    tmp_path: Path,
    workspace_modules: object,
) -> None:
