...
from datapaw.host.core import model as model_module
...
def test_datapaw_model_env_overrides_shared_values() -> None:
    result = model_module.build_model_from_env(
        {
            "DATAPAW_MODEL_PROVIDER": "openai",
            "DATAPAW_MODEL_NAME": "cli-model",
            "DATAPAW_MODEL_API_KEY": "cli-key",
            "DATAPAW_MODEL_BASE_URL": "https://cli.example/v1",
            "LLM_MODEL": "shared-model",
            "OPENAI_API_KEY": "shared-key",
            "OPENAI_BASE_URL": "https://shared.example/v1",
        },
...
def test_dashscope_provider_remains_supported() -> None:
    result = model_module.build_model_from_env(
        {
            "DATAPAW_MODEL_PROVIDER": "dashscope",
            "DATAPAW_MODEL_NAME": "qwen-model",
            "DATAPAW_MODEL_API_KEY": "dashscope-key",
        },
    )
...
def test_missing_model_config_reports_alternatives_without_secret() -> None:
    ...
    assert "DATAPAW_MODEL_NAME (or LLM_MODEL)" in message
    assert "DATAPAW_MODEL_API_KEY" not in message
...
def test_missing_model_and_api_key_report_both_fallbacks() -> None:
    ...
    message = str(exc_info.value)
    assert "DATAPAW_MODEL_NAME (or LLM_MODEL)" in message
    assert "DATAPAW_MODEL_API_KEY (or OPENAI_API_KEY)" in message
...
def test_unsupported_provider_does_not_expose_api_key() -> None:
    ...
    with pytest.raises(RuntimeError) as exc_info:
        model_module.build_model_from_env(
            {
                "DATAPAW_MODEL_PROVIDER": "unsupported",
                "DATAPAW_MODEL_NAME": "model",
                "DATAPAW_MODEL_API_KEY": secret,
            },
        )

    message = str(exc_info.value)
    assert "Unsupported DATAPAW_MODEL_PROVIDER='unsupported'" in message
    assert secret not in message
