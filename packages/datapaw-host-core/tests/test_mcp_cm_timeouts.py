...
from datapaw.host.core.agent.toolkit import build_datapaw_toolkit
from datapaw.host.core.mcp_cm import (
    CM_MCP_READ_TIMEOUT,
...
async def test_build_datapaw_toolkit_records_cm_mcp_prefix(monkeypatch) -> None:
    monkeypatch.delenv("DATAPAW_SPAWN_SUBAGENT_ENABLED", raising=False)
    monkeypatch.setenv("DATAPAW_API_TOKEN", "mcp-runtime-secret")
    client = _mcp_client(
        url=_CM_MCP_URL,
    )
    ...
    toolkit = await build_datapaw_toolkit(
        RuntimeStateManager(),
        workspace=FakeWorkspace(),
    )

    assert toolkit._datapaw_cm_mcp_tool_prefixes == {"mcp__context-manager__"}
    assert client.execution_timeout == CM_MCP_READ_TIMEOUT
...
async def test_plan_mode_allows_non_sql_mcp_tools_only(caplog, monkeypatch) -> None:
    ...
    toolkit.set_datapaw_mode("agent")
    agent_names = {
        schema["function"]["name"]
        for schema in await toolkit.get_tool_schemas(["agent"])
    }
...
async def test_spawn_subagent_can_be_disabled(monkeypatch) -> None:
    monkeypatch.setenv("DATAPAW_SPAWN_SUBAGENT_ENABLED", "0")
