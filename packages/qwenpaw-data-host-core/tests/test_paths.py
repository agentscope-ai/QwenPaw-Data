...
from qwenpaw_data.host.core.paths import (
    Paths,
    host_root,
...
def test_resolve_qwenpaw_data_home_prefers_explicit_path(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("QWENPAW_DATA_HOME", str(tmp_path / "from-env"))

    assert resolve_qwenpaw_data_home(tmp_path / "explicit") == (
        tmp_path / "explicit"
    ).resolve()


def test_resolve_qwenpaw_data_home_uses_trimmed_environment(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    expected = tmp_path / "from-env"
    monkeypatch.setenv("QWENPAW_DATA_HOME", f"  {expected}  ")

    assert resolve_qwenpaw_data_home() == expected.resolve()


def test_resolve_qwenpaw_data_home_uses_default_for_blank_environment(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fake_user_home = tmp_path / "user"
    expected = fake_user_home / ".qwenpaw-data"
    monkeypatch.setenv("QWENPAW_DATA_HOME", "   ")
...
def test_resolve_qwenpaw_data_home_expands_user_without_creating_directories(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fake_user_home = tmp_path / "user"
    ...
    resolved = resolve_qwenpaw_data_home("~/custom-qwenpaw-data")

    assert resolved == (fake_user_home / "custom-qwenpaw-data").resolve()
