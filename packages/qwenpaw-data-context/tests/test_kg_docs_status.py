...
from qwenpaw_data.context.blocking_io import BlockingIOGovernor
...
class KgDocPathConfigTest(unittest.TestCase):
    def test_default_doc_storage_dir_uses_qwenpaw_data_home(self) -> None:
        old_home = os.environ.get("QWENPAW_DATA_HOME")
        old_storage = os.environ.get("DOC_STORAGE_DIR")
        temp_dir = tempfile.TemporaryDirectory()
        import context_manager.config as config_module

        try:
            os.environ["QWENPAW_DATA_HOME"] = temp_dir.name
            ...
            if old_home is None:
                os.environ.pop("QWENPAW_DATA_HOME", None)
            else:
                os.environ["QWENPAW_DATA_HOME"] = old_home
...
class KgIngestCachePathTest(unittest.TestCase):
    def test_default_cache_and_report_paths_use_qwenpaw_data_home(self) -> None:
        old_home = os.environ.get("QWENPAW_DATA_HOME")
        temp_dir = tempfile.TemporaryDirectory()
        import qwenpaw_data.context.paths as paths_module
