...
def _build_security_app(
    ...
    *,
    ...
    monkeypatch.setenv("QWENPAW_DATA_API_TOKEN", "security-test-token")
    monkeypatch.delenv("QWENPAW_DATA_API_KEYS", raising=False)
    monkeypatch.setenv(
        "QWENPAW_DATA_SECURITY_AUDIT_LOG",
        str(tmp_path / "security-audit.jsonl"),
    )
    monkeypatch.setenv("QWENPAW_DATA_API_ALLOWED_HOSTS", "testserver")
...
def test_authentication_failures_trigger_penalty_box(
    ...
    monkeypatch.setenv("QWENPAW_DATA_AUTH_FAILURE_LIMIT", "2")
    monkeypatch.setenv("QWENPAW_DATA_AUTH_FAILURE_WINDOW_SECONDS", "3600")
    client = TestClient(_build_security_app(monkeypatch, tmp_path))
...
def test_per_principal_scope_rate_limit(
    ...
    monkeypatch.setenv("QWENPAW_DATA_RATE_LIMIT_QUERY", "2")
    monkeypatch.setenv("QWENPAW_DATA_RATE_LIMIT_WINDOW_SECONDS", "3600")
    client = TestClient(_build_security_app(monkeypatch, tmp_path))
    auth = {"Authorization": "Bearer security-test-token"}
...
def test_forwarded_ip_is_used_only_for_trusted_proxy(
    ...
    monkeypatch.setenv("QWENPAW_DATA_TRUSTED_PROXIES", "10.0.0.0/8")
