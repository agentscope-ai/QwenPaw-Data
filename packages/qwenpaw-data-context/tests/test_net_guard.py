...
def test_callbacks_are_deny_by_default(
    ...
    monkeypatch.delenv("QWENPAW_DATA_CALLBACK_ALLOWLIST", raising=False)
    with pytest.raises(CallbackUrlError):
        ensure_safe_callback_url(url)
...
def test_internal_callback_configuration_is_loopback_only(
    ...
    monkeypatch.setenv(
        "QWENPAW_DATA_INTERNAL_CALLBACK_ORIGINS",
        "http://10.0.0.8:8765",
    )
...
def test_internal_callback_dns_must_remain_loopback(
    ...
    monkeypatch.delenv("QWENPAW_DATA_INTERNAL_CALLBACK_ORIGINS", raising=False)
    monkeypatch.setattr(
        socket,
        "getaddrinfo",
...
def test_allowlisted_private_ip_literal_is_explicit_opt_in(
    ...
    monkeypatch.setenv("QWENPAW_DATA_CALLBACK_ALLOWLIST", "http://127.0.0.1:9000")
