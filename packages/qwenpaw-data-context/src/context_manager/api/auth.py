...
支持一个向后兼容的全权限 ``QWENPAW_DATA_API_TOKEN``，以及
``QWENPAW_DATA_API_KEYS`` 中带独立 scope 的 API key。配置任一凭证后，除健康
检查等豁免路径外的所有 HTTP 请求必须携带匹配的 Bearer token。

前端将 token 保存在 sessionStorage 的 ``qwenpaw_data_auth_token`` 键中，
...
API_KEYS_ENV = "QWENPAW_DATA_API_KEYS"
CLIENT_API_TOKEN_ENV = "QWENPAW_DATA_CLIENT_API_TOKEN"
...
def get_configured_api_token() -> str:
    ...
    return (os.environ.get("QWENPAW_DATA_API_TOKEN") or "").strip()
...
def internal_callback_auth_headers(callback_url: str) -> dict[str, str]:
    ...
    QwenPaw Data API credential to those destinations.
    """
    parsed = urlparse(callback_url)
    trusted_origins = {
        origin.strip().rstrip("/").lower()
        for origin in (
            os.environ.get("QWENPAW_DATA_INTERNAL_CALLBACK_ORIGINS")
...
def install_api_token_auth(
    ...
    *,
    ...
    if not enabled:
        log.warning(
            "未配置 QwenPaw Data API 凭证：API 无认证运行，仅适合绑定 127.0.0.1 "
            "的本机部署；对外暴露前必须配置 QWENPAW_DATA_API_TOKEN 或 QWENPAW_DATA_API_KEYS。"
        )
