...
from qwenpaw_data.context.paths import security_audit_log_path
...
def configured_cors_origins(raw: str | None = None) -> list[str]:
    ...
    source = raw if raw is not None else os.environ.get("QWENPAW_DATA_CORS_ORIGINS")
...
def configured_api_hosts(raw: str | None = None) -> list[str]:
    ...
    source = raw if raw is not None else os.environ.get("QWENPAW_DATA_API_ALLOWED_HOSTS")
    ...
    for value in values:
        ...
        if (
            ...
            raise ValueError(f"invalid QWENPAW_DATA_API_ALLOWED_HOSTS entry: {value!r}")
        try:
        ...
        try:
            ...
            raise ValueError(f"invalid QWENPAW_DATA_API_ALLOWED_HOSTS entry: {value!r}") from exc
...
def _trusted_proxy_networks(
    ...
    for value in (os.environ.get("QWENPAW_DATA_TRUSTED_PROXIES") or "").split(","):
        if not value.strip():
            continue
        try:
            networks.append(ipaddress.ip_network(value.strip(), strict=False))
        except ValueError as exc:
            raise ValueError(f"invalid QWENPAW_DATA_TRUSTED_PROXIES entry: {value!r}") from exc
    if len(networks) > 100:
        raise ValueError("QWENPAW_DATA_TRUSTED_PROXIES cannot contain more than 100 entries")
...
class SecurityAudit:
    ...
    def _get_logger(self) -> logging.Logger:
        ...
        with self._lock:
            ...
            audit_logger = logging.getLogger(f"qwenpaw_data.security.audit.{self.path}")
...
class SecurityControls:
    ...
    def from_env(cls, allowed_origins: list[str] | None = None) -> "SecurityControls":
        ...
        audit_path = Path(
            os.environ.get("QWENPAW_DATA_SECURITY_AUDIT_LOG") or security_audit_log_path()
        ).expanduser()
        return cls(
            ...
            auth_failures=FailureLimiter(
                _env_int("QWENPAW_DATA_AUTH_FAILURE_LIMIT", 10),
                _env_int("QWENPAW_DATA_AUTH_FAILURE_WINDOW_SECONDS", 60),
            ),
            rate_limiter=TokenBucketLimiter(),
            audit=SecurityAudit(audit_path),
            rate_window_seconds=_env_int("QWENPAW_DATA_RATE_LIMIT_WINDOW_SECONDS", 60),
            scope_limits={
                "query": _env_int("QWENPAW_DATA_RATE_LIMIT_QUERY", 120),
                "write": _env_int("QWENPAW_DATA_RATE_LIMIT_WRITE", 60),
                "manage": _env_int("QWENPAW_DATA_RATE_LIMIT_MANAGE", 30),
                "credentials:manage": _env_int("QWENPAW_DATA_RATE_LIMIT_CREDENTIALS", 20),
                "authenticated": _env_int("QWENPAW_DATA_RATE_LIMIT_AUTHENTICATED", 60),
            },
        )
