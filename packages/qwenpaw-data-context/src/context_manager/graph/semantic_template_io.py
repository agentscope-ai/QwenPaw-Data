...
def _validate_xlsx_archive(path: Path) -> None:
    ...
    max_entries = _positive_env_int("QWENPAW_DATA_XLSX_MAX_ENTRIES", 1000)
    max_uncompressed = (
        _positive_env_int("QWENPAW_DATA_XLSX_MAX_UNCOMPRESSED_MB", 200)
        * 1024
        * 1024
    )
    max_ratio = _positive_env_int("QWENPAW_DATA_XLSX_MAX_COMPRESSION_RATIO", 100)
    try:
        with zipfile.ZipFile(path) as archive:
            ...
            if unpacked > max_uncompressed:
                raise ValueError(
                    ...
                    "QWENPAW_DATA_XLSX_MAX_UNCOMPRESSED_MB"
                )
            if unpacked > max_ratio * max(1, packed):
                raise ValueError(
                    "xlsx compression ratio exceeds "
                    "QWENPAW_DATA_XLSX_MAX_COMPRESSION_RATIO"
                )
...
def _load_sheet_rows(wb: Any, sheet_name: str) -> list[dict[str, Any]]:
    ...
    out: list[dict[str, Any]] = []
    max_rows = _positive_env_int("QWENPAW_DATA_XLSX_MAX_ROWS_PER_SHEET", 200000)
    max_cells = _positive_env_int("QWENPAW_DATA_XLSX_MAX_CELLS_PER_SHEET", 2000000)
    cell_count = len(first_row)
    for r in rows:
        if len(out) >= max_rows:
            raise ValueError(
                f"sheet {sheet_name!r} exceeds QWENPAW_DATA_XLSX_MAX_ROWS_PER_SHEET"
            )
        cell_count += len(r)
        if cell_count > max_cells:
            raise ValueError(
                f"sheet {sheet_name!r} exceeds QWENPAW_DATA_XLSX_MAX_CELLS_PER_SHEET"
            )
