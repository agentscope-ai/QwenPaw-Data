...
``QWENPAW_DATA_CALLBACK_ALLOWLIST`` origin (scheme, host and port). The built-in
...
def _allowed_origins() -> frozenset[str]:
    origins: set[str] = set()
    for entry in (os.environ.get("QWENPAW_DATA_CALLBACK_ALLOWLIST") or "").split(","):
        if entry.strip():
...
def _internal_callback_urls() -> frozenset[str]:
    raw = os.environ.get("QWENPAW_DATA_INTERNAL_CALLBACK_ORIGINS")
    ...
    for value in values:
        ...
        if not is_loopback:
            raise CallbackUrlError(
                "QWENPAW_DATA_INTERNAL_CALLBACK_ORIGINS must contain only loopback origins"
            )
...
def _normalize_callback_url(url: str) -> tuple[str, str, int, bool]:
    ...
    if not internal and origin not in _allowed_origins():
        raise CallbackUrlError(
            ...
            "QWENPAW_DATA_CALLBACK_ALLOWLIST"
        )
...
async def post_safe_callback(
    ...
    *,
    ...
    max_request = _env_int(
        "QWENPAW_DATA_CALLBACK_MAX_REQUEST_BYTES",
        65_536,
        minimum=1_024,
        maximum=1_048_576,
    )
    max_response = _env_int(
        "QWENPAW_DATA_CALLBACK_MAX_RESPONSE_BYTES",
        65_536,
        minimum=1_024,
        maximum=1_048_576,
    )
    total_timeout = _env_int(
        "QWENPAW_DATA_CALLBACK_TIMEOUT_SECONDS",
        10,
        minimum=1,
        maximum=60,
    )
