...
所有 DataBridge 侧的运行时状态统一归属到 ``${QWENPAW_DATA_HOME:-~/.qwenpaw-data}/data-bridge/``，
敏感配置归入 ``${QWENPAW_DATA_HOME:-~/.qwenpaw-data}/.secrets/``。

参考：QwenPaw Data 状态目录统一设计文档。
"""
...
QWENPAW_DATA_HOME = "QWENPAW_DATA_HOME"
_DEFAULT_HOME = Path.home() / ".qwenpaw-data"


def resolve_qwenpaw_data_home() -> Path:
    """解析 QWENPAW_DATA_HOME 根目录。

    优先级：``QWENPAW_DATA_HOME`` 环境变量 > 默认 ``~/.qwenpaw-data``。
    """
    raw = os.environ.get(QWENPAW_DATA_HOME)
    if raw:
...
def data_bridge_root() -> Path:
    """DataBridge 顶层状态目录。"""
    return resolve_qwenpaw_data_home() / "data-bridge"
...
def secrets_dir() -> Path:
    """跨域敏感配置目录。"""
    return resolve_qwenpaw_data_home() / ".secrets"
