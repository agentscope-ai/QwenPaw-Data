...
@dataclass(eq=False)
class QwenPawDataError(Exception):
    code: str
    message: str
    http_status: int = 500
...
class ResourceBudgetExceeded(QwenPawDataError):
    def __init__(
        self,
        resource: str,
        *,
...
class UpstreamServiceError(QwenPawDataError):
...
class StateConflictError(QwenPawDataError):
...
def install_error_handlers(app: Any) -> None:
    ...
    async def _handle_qwenpaw_data_error(
        _request: Request,
        error: QwenPawDataError,
    ) -> JSONResponse:
    ...
    app.add_exception_handler(QwenPawDataError, _handle_qwenpaw_data_error)
