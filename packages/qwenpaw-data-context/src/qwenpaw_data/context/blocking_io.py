...
log = logging.getLogger("qwenpaw_data.context.blocking_io")
...
def blocking_pool_configs_from_env() -> dict[BlockingPool, BlockingPoolConfig]:
    ...
    for pool, (workers, queue, queue_timeout, operation_timeout) in defaults.items():
        prefix = f"QWENPAW_DATA_BLOCKING_{pool.value.upper()}"
...
class _GovernedPool:
    def __init__(self, name: BlockingPool, config: BlockingPoolConfig) -> None:
        ...
        self.executor = ThreadPoolExecutor(
            ...
            thread_name_prefix=f"qwenpaw-data-{name.value}",
        )
    ...
    def __init__(
        ...
        *,
        ...
        self.shutdown_timeout_seconds = (
            ...
            else _positive_float_env("QWENPAW_DATA_BLOCKING_SHUTDOWN_TIMEOUT_SECONDS", 10.0)
        )
        self.closed = False
    ...
    def submit(
        ...
        **kwargs: Any,
        ...
        task = asyncio.create_task(
            ...
            name=f"qwenpaw-data-blocking-{pool.value}-{operation}",
        )
