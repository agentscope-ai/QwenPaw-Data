...
def _authentication_configured() -> bool:
    if (os.environ.get("QWENPAW_DATA_API_TOKEN") or "").strip():
        return True
    raw_keys = (os.environ.get("QWENPAW_DATA_API_KEYS") or "").strip()
    if not raw_keys:
        return False
    try:
...
def main() -> int:
    ...
    p.add_argument(
        "--limit-concurrency",
        type=int,
        default=_positive_int_env("QWENPAW_DATA_HTTP_MAX_CONCURRENCY", 128),
    ...
    if (
        ...
        print(
            ...
            "Configure QWENPAW_DATA_API_TOKEN or QWENPAW_DATA_API_KEYS, or keep both API "
