# QwenPaw Data GAAP Demo Assets

This document describes the bundled GAAP demo assets shipped inside
`qwenpaw-data-context`. The assets are used by the docker-compose one-shot demo in
...
|---|---|---|
| `context_manager/demo/assets/demo_semantic_config.xlsx` | Semantic-layer workbook (domains, metrics, dimensions, bindings) | `qwenpaw-data semantic import` |
| `context_manager/demo/assets/seed-postgres.sql` | Idempotent PostgreSQL seed script: schema + 475 GAAP rows | `psql` or `qwenpaw-data demo seed` |

## Why the assets live here

The demo must work without a local `QwenPaw-Data` source checkout. Shipping the
assets inside `qwenpaw-data-context` means any environment with the PyPI package can
...
`qwenpaw-data-context` was chosen because the context service consumes both assets:
...
```bash
cd packages/qwenpaw-data-context
...
4. Bump `qwenpaw-data-context` and publish a new wheel.

## Release note

Demo assets were introduced in `qwenpaw-data-context==0.2.1`.
